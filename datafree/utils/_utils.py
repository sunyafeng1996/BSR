...
class LabeledImagePool(object):
    def __init__(self, root, num_classes):
        self.root = os.path.abspath(root)
        os.makedirs(self.root, exist_ok=True)
        self._idx = torch.zeros((num_classes))
        self.features_pool={}
        self.categories=list(range(num_classes))
        for i in range(num_classes):
            os.makedirs(self.root+'/'+str(i)+'/', exist_ok=True)
            self.features_pool[i]=None

    def add(self, imgs, targets,features):
        for i in range(targets.size(0)):
            fn=self.root+'/'+str(targets[i].item())+'/'+"%d.png"%(self._idx[targets[i].item()].item())
            save_image_batch(imgs[i:i+1,:,:,:], fn, pack=False)
            if self.features_pool[targets[i].item()] is None:
                self.features_pool[targets[i].item()]={}
                self.features_pool[targets[i].item()][fn]=features[i].cpu().detach().numpy()
            else:
                self.features_pool[targets[i].item()][fn]=features[i].cpu().detach().numpy()
            self._idx[targets[i].item()]+=1

    def get_dataset(self, transform=None):
        return LabeledImageDataset(self.root, transform=transform)
