...
from datafree.utils import LabeledImagePool, DataIter
...
class BSRSynthesizer(BaseSynthesis):
    def __init__(self, teacher, student, generator, nz, num_classes, img_size,
        ...
        self.num_labels=torch.zeros((num_classes))
    ...
    def synthesize(self, targets=None):
        ...
        for it in range(self.iterations):
            ...
            # Inversion Loss
            #############################################
            t_out,t_fea = self.teacher(inputs_aug,return_features=True)       
            ...
            with torch.no_grad():
                if best_cost > loss.item() or best_inputs is None:
                    ...
                    best_fea = t_fea.detach()
                    
                    self.bloss = loss.item()
                    self.bloss_adv = loss_adv.item()
                    self.bloss_oh = loss_oh.item()
                    self.bloss_bn = loss_bn.item()
        ...
        # def add(self, imgs, targets=None) 需要修改这个函数
        t_out = self.teacher(self.aug(best_inputs.detach()))
        targets = torch.argmax(t_out, dim=-1)
        targets = targets.to(self.device)
        
        for cls in range(self.num_classes):
            self.num_labels[cls]=self.num_labels[cls]+(targets==cls).sum().item()
